...
class FusionAuthPriceCalculator {
  ...
  _buildPurchaseUrl(plan = this.plan) {
    return `https://${this.accountHost}/account/purchase/start?hosting=${this.hosting}&plan=${plan}&renewal=${this._billingIntervalReadable()}&monthlyActiveUsers=${this.monthlyActiveUserSlider.value}`;
  }
  ...
  _redraw() {
    ...
    if (this.hosting === 'ha-cloud') {
      ...
      this.communityButton.href = this._buildPurchaseUrl('Community');
      this.communityButton.innerText = 'Get started';
    }
